...
import {applicationERCs, environmentConfigKeys, oauthServerConfigKeys} from "./constants.js";
...
const clientId = getOAuthConfigByKey(applicationERCs.OAUTH_SERVER_ERC,oauthServerConfigKeys._OAUTH2_HEADLESS_SERVER_CLIENT_ID);

const clientSecret = getOAuthConfigByKey(applicationERCs.OAUTH_SERVER_ERC,oauthServerConfigKeys._OAUTH2_HEADLESS_SERVER_CLIENT_SECRET);

const uri = getOAuthConfigByKey(applicationERCs.OAUTH_SERVER_ERC,oauthServerConfigKeys._OAUTH2_TOKEN_URI);
	...
	const prom = new Promise((resolve, reject) => {
		...
		if (cachedData) {
			...
			try {
				...
				axios.post(tokenEndpoint, requestBody, {headers}).then((response) => {
