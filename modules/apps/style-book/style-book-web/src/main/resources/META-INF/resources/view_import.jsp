...
>
	<liferay-frontend:edit-form-body>
		...
		<%
		List<String> invalidStyleBookEntryImporterResultNames = importStyleBookDisplayContext.getStyleBookEntryImporterResultNames(StyleBookEntryImporterResult.Status.INVALID);
		%>

		<c:if test="<%= ListUtil.isNotEmpty(invalidStyleBookEntryImporterResultNames) %>">
			<clay:alert
				dismissible="<%= true %>"
				displayType="warning"
				message='<%= LanguageUtil.format(request, "the-following-style-books-could-not-be-imported-x", "<strong>" + StringUtil.merge(invalidStyleBookEntryImporterResultNames, StringPool.COMMA_AND_SPACE) + "</strong>", false) %>'
			/>
		</c:if>
